...
export async function POST(req: Request) {
  ...
  let corpo: { congId?: number; cargo?: string; pessoaId?: number | null; alunoId?: number };
  try {
  ...
  const { congId, cargo, pessoaId, alunoId } = corpo ?? {};
  ...
  // Três formas de chamar: pessoa existente, aluno a promover, ou vago.
  const vago = (pessoaId === null || pessoaId === undefined) && alunoId === undefined;
  if (!vago && !Number.isInteger(pessoaId) && !Number.isInteger(alunoId)) {
    return erro("Informe a pessoa (ou aluno), ou null para deixar vago.", 400);
  }
  ...
  return responder(async () => {
    ...
    const resultado = await prisma.$transaction(async (tx) => {
      ...
      // Aluno a promover vira Pessoa aqui; pessoa existente é usada direto.
      const alvoId = Number.isInteger(alunoId)
