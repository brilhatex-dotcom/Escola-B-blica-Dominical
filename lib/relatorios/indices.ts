...
/* ------------------------------------------------------------------ *
 * O Radar — os cinco componentes, num eixo cada
 * ------------------------------------------------------------------ */

/** Um eixo do radar. `valor: null` quando aquele componente não pôde ser medido. */
export interface EixoRadar {
  eixo: string;
  chave: keyof ComponentesIGS;
  valor: number | null;
}

/**
 * Os rótulos do radar, na MESMA ordem para toda congregação.
 *
 * ============================================================================
 * "FALTOSOS" VIRA "PRESENÇA CONTÍNUA" NO EIXO — O NÚMERO JÁ ESTÁ INVERTIDO
 *
 * O componente `faltosos` é uma penalidade calculada como `100 - proporção de
 * faltosos`: 100 já significa "ninguém sumindo". Rotulá-lo de "Faltosos" no
 * radar faria uma congregação saudável (nota 95 no eixo) parecer que tem MUITA
 * falta, porque no resto do sistema número alto de falta é ruim. O rótulo
 * segue o SINAL do número, não o nome interno do componente.
 * ============================================================================
 */
const ROTULOS_RADAR: Array<{ chave: keyof ComponentesIGS; eixo: string }> = [
  { chave: "frequencia", eixo: "Frequência" },
  { chave: "regularidade", eixo: "Regularidade" },
  { chave: "tendencia", eixo: "Tendência" },
  { chave: "visitantes", eixo: "Visitantes" },
  { chave: "faltosos", eixo: "Presença contínua" },
];

/** Os componentes prontos para desenhar — sempre os 5 eixos, na mesma ordem. */
export function paraRadar(componentes: ComponentesIGS): EixoRadar[] {
  return ROTULOS_RADAR.map(({ chave, eixo }) => ({ eixo, chave, valor: componentes[chave] }));
}

/* ------------------------------------------------------------------ *
 * O Comparativo — quantas congregações fazem sentido lado a lado
 * ------------------------------------------------------------------ */

export const COMPARATIVO_MINIMO = 2;
export const COMPARATIVO_MAXIMO = 4;

/**
 * `null` quando a seleção serve; senão, a frase pronta para mostrar.
 *
 * ============================================================================
 * O TETO É DE LEGIBILIDADE, NÃO DE DESEMPENHO
 *
 * O radar aceitaria 14 polígonos sobrepostos sem reclamar — e o resultado
 * seria ilegível, uma nuvem de linhas coloridas em cima umas das outras. Quatro
 * é quanto dá para diferenciar por cor à primeira vista, inclusive no celular.
 * ============================================================================
 */
export function validarSelecaoComparativo(ids: number[]): string | null {
  const distintos = new Set(ids);
  if (distintos.size < COMPARATIVO_MINIMO) {
    return `Escolha pelo menos ${COMPARATIVO_MINIMO} congregações para comparar.`;
  }
  if (distintos.size > COMPARATIVO_MAXIMO) {
    return `Escolha no máximo ${COMPARATIVO_MAXIMO} congregações — mais que isso e o gráfico vira uma nuvem ilegível.`;
  }
  return null;
}
